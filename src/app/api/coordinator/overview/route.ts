...
export async function GET(req: Request) {
  try {
    ...
    let myInstitutionId = institutionIdParam;
    ...
    if (!myInstitutionId) {
      ...
      myInstitutionId = dbUser?.institutionId || null;
    }

    if (!myInstitutionId) {
      console.log("No institutionId found for user:", user.userId);
    ...
    const [students, teachers, books, sessions7d, avgProgressAgg, avgQuizAgg] = await Promise.all([
      prisma.user.count({ where: { institutionId: myInstitutionId, role: "STUDENT" } }),
      prisma.user.count({ where: { institutionId: myInstitutionId, role: { in: ["TEACHER", "COORDINATOR"] } } }),
      prisma.book.count(),
    ...
    const topBooks = await prisma.$queryRaw<Array<{ bookId: string; reads: number; title: string; author: string }>>`
      ...
      GROUP BY rs.bookId, b.title, b.author
      ORDER BY "reads" DESC
      LIMIT 6
    `;
    ...
    const latestAttempts = await prisma.activityAttempt.findMany({
      where: { 
        user: { institutionId: myInstitutionId, id: { not: DEMO_USER_ID } }, 
    ...
    return NextResponse.json({
      ...
      topBooks: topBooks.map(b => ({ ...b, reads: Number(b.reads) })),
      progressByGrade: progressByGrade.map((r) => ({ grade: r.grade, avgProgress: Math.round(Number(r.avgProgress) || 0) })),
    ...
    console.error("Error in coordinator overview:", error);
