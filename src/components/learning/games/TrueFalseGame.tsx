...
export function TrueFalseGame({ statements, onComplete, onExit }: TrueFalseGameProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center h-full">
        <div className="bg-card dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-xl border-2 border-indigo-50 dark:border-indigo-900/20">
          ...
          <p className="text-muted-foreground dark:text-gray-400 italic">No hay suficientes afirmaciones para este libro.</p>
  ...
  if (isFinished) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center h-full">
        <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="bg-card p-10 rounded-[3rem] shadow-2xl max-w-md w-full border-b-8 border-indigo-500/50/30">
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4 flex flex-col items-center justify-center h-full gap-8">
      {/* Header Info */}
      <div className="w-full flex justify-between items-center bg-muted p-4 rounded-2xl">
        <div className="bg-card px-4 py-2 rounded-xl shadow-sm font-bold text-indigo-400">Pregunta {currentIndex + 1} / {statements.length}</div>
        <div className={`flex items-center gap-2 font-mono text-xl ${timeLeft < 5 ? "text-red-500 animate-pulse" : "text-muted-foreground"}`}>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit={{ x: -50, opacity: 0 }}
          className={`w-full min-h-[200px] bg-card border-4 p-8 rounded-[2.5rem] shadow-xl flex items-center justify-center text-center transition-colors duration-300 ${
            showFeedback === true ? "border-green-500 bg-green-50" : 
            showFeedback === false ? "border-red-500 bg-red-50" : "border-indigo-500/50/20"
          }`}
        >
