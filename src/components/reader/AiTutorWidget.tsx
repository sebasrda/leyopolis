          ...
          >
            ...
            >
              ...
              <Button 
                ...
                className="h-8 w-8 hover:bg-card/20 text-white rounded-full cursor-pointer"
            ...
            <div 
              className="flex-1 overflow-y-auto p-4 bg-muted space-y-4 scrollbar-thin scrollbar-thumb-gray-200"
              onPointerDown={(e) => e.stopPropagation()}
            >
              {/* Educational disclaimer notice */}
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-2 text-[10px] text-amber-600 font-medium text-center">
              ...
              {messages.map((msg) => (
                ...
                >
                  <div className={cn(
                    ...
                    msg.role === 'user' ? "bg-gray-800 text-white" : "bg-card text-blue-600 border border-blue-100"
                  )}>
              ...
              {isLoading && (
                <div className="flex gap-3 max-w-[85%]">
                   <div className="w-8 h-8 rounded-full bg-card text-blue-600 border border-blue-100 flex items-center justify-center shrink-0 shadow-sm">
                    <Bot size={16} />
                  </div>
                  <div className="bg-card p-3 rounded-2xl rounded-tl-none border border-border shadow-sm flex items-center gap-2">
            ...
            <form onSubmit={handleSubmit} className="p-3 bg-card border-t border-border flex gap-2 shrink-0" onPointerDown={(e) => e.stopPropagation()}>
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Pregunta algo sobre el libro..."
                className="flex-1 bg-muted text-sm rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-card transition-all text-foreground placeholder:text-gray-400"
              />
              <Button 
                type="submit" 
                size="icon" 
