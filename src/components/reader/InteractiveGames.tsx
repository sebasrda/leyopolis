...
export default function InteractiveGames({ isOpen, onClose, bookTitle, evaluations = [], defaultGame = null }: InteractiveGamesProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="w-full max-w-5xl h-[90vh] bg-card rounded-3xl overflow-hidden shadow-2xl flex flex-col relative">
        ...
        <div className="h-20 bg-gradient-to-r from-indigo-600 to-purple-600 flex items-center justify-between px-8 text-white shrink-0">
          ...
          <Button 
            ...
            size="icon" 
            onClick={onClose} 
            className="hover:bg-card/20 text-white rounded-full h-10 w-10"
          >
            <X className="h-6 w-6" />
          </Button>
        </div>
        ...
        {/* Content Area */}
        <div className="flex-1 overflow-hidden bg-indigo-500/10">
          {!activeGame ? (
...
function GameMenu({ onSelectGame }: { onSelectGame: (g: GameType) => void }) {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-8 overflow-y-auto">
      <h3 className="text-3xl font-bold text-foreground mb-12 text-center">¡Elige tu Actividad!</h3>
          ...
          >
            <Card className="h-full overflow-hidden border-none shadow-xl hover:shadow-2xl transition-all duration-300 flex flex-col">
              ...
              <CardContent className="p-6 text-center flex-1 flex flex-col">
                <h4 className="text-xl font-bold text-foreground mb-2">{game.title}</h4>
  ...
  if (showResult) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center max-w-lg mx-auto">
        ...
        <h3 className="text-3xl font-bold text-foreground mb-2">¡Juego Completado!</h3>
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      <div className="mb-8 flex items-center justify-between">
        <span className="text-muted-foreground font-bold">Pregunta {currentQuestion + 1} de {activeQuestions.length}</span>
        <div className="flex items-center gap-2 bg-indigo-500/20 px-4 py-1 rounded-full text-indigo-300 font-bold">
      ...
      <Card className="border-none shadow-lg mb-8 bg-card overflow-hidden">
          ...
          <motion.button
            ...
            className={`p-6 rounded-xl text-left font-semibold text-lg transition-all border-2 ${
              selectedAnswer === index
                ...
                : "bg-card border-border hover:border-indigo-400 hover:shadow-md text-foreground"
            }`}
          >
        ...
        {cards.map((card, index) => (
          ...
          >
            <div className={`relative w-full h-full transition-transform duration-500 transform-style-3d ${
              ...
              {/* Back (Revealed) */}
              <div className={`absolute w-full h-full bg-card rounded-xl flex items-center justify-center backface-hidden rotate-y-180 shadow-xl border-4 ${
